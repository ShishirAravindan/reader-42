...
// Supports:
//   - STORED (method 0)
//   - DEFLATE (method 8) via DecompressionStream
//   - Standard 32-bit central directory
//
// Does not support:
//   - Encrypted entries
//   - ZIP64 (EPUBs are tiny in practice)
//   - Multi-disk archives
//
// References used: PKWARE APPNOTE 6.3.x (sections 4.3.6, 4.3.12, 4.3.16).
...
export interface ZipEntry {
  /** UTF-8 file name (forward-slash separated). */
  name: string;
...
export class Zip {
  ...
  private findDataOffset(entry: ZipEntry): number {
    ...
    const sig = this.view.getUint32(off, true);
    if (sig !== SIG_LFH) throw new Error('zip: bad LFH signature');
...
function findEocd(view: DataView, length: number): number {
  // EOCD is at the end, comment may follow up to 65535 bytes.
...
async function inflateRaw(compressed: Uint8Array): Promise<Uint8Array> {
  // DecompressionStream supports 'deflate-raw' in modern browsers + Bun.
